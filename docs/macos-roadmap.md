...
**本地图片解码修复(2026-06-29)** ✅ **PNG 图片端到端渲染**(截图实证:`$r('app.media.icon')` 应用图标正确显示)。**根因**:`image_utils.cpp::RegisterPluginServer` 的 `_APPLE`(mac)分支用 `pluginPaths = {"./"}` 让 `PluginServer` 扫文件系统找 `.so` 插件 + metadata,但 mac 把 codec 插件(`libextplugin` 的 `ExtDecoder`/`ExtEncoder`、gif)**静态链接进 ace_macos**(无 `.so`)→ 扫不到 → 不注册任何解码器 → `ImageSource` 报 "No extended decoder available"。**修复**:mac 改用**空 `pluginPaths`**(同 Android/iOS),走 `PluginFw::Register` 的**静态注册**路径(注册编译进来的 codec)。单行根因,影响所有本地/资源图片。
