...
> 声明式 `@Entry @Component` → 方舟运行时 → RenderService → NSOpenGLContext/CAOpenGLLayer → 屏幕。

![ArkUI 在 macOS 原生渲染](screenshots/M1_arkui_rendered.png)

> 上图:`ace_macos` 原生窗口里,一个标准 ArkUI 页面(`Column` 里三个 `Row`,蓝/橙/绿、圆角、居中)被 RenderService 真实渲染上屏。从 GN 编译到这一帧,全链由真构建系统(gn/ninja + Xcode clang)产出,无手工 `.o`、`third_party` 仅 1 处必要去重。
...
| 仓 | 说明 |
|--|--|
| **ace_engine** | `adapter/macos` 窗口层 + 整框架 mac 适配 + 渲染管线修复(主体) |
...
## 已知项 / 下一步
- **文字渲染**:示例页面用纯色块验证渲染通路(故意避开字体)。默认 gn args 关了字体后端(`skia_use_fonthost_mac=false`);开 CoreText 字体端口需把 skia 的 `fontmgr_symbol_load` 对 OHOS fontconfig 的依赖 mac/arkui-x gate 掉(本仓 skia.patch 已含该 gate),再 `skia_use_fonthost_mac=true` 重编。
